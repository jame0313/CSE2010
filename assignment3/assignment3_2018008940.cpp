...
int main(){
	...
	Node tmp = NULL; //임시 노드 선언 (명령어마다 쓰임새 다름) 
	char command; //명령어 문자 
	//명령어가 e일경우 종료
	while(command=getchar(), command!='e'){
		...
		switch(command){
			//삽입 
				...
				tmp = makeEmptyList(tmp);//노드 생성 
				tmp->value = value; //생성한 노드에 값 담기 
				insert(dest,single_list,tmp); //삽입
				break;
			//삭제 
			case 'd': 
				//dest: 목적지 노드의 값
			...
			case 'f':
				...
				//tmp: 검색한 대상 
				scanf("%d",&dest);
				tmp = findPrev(dest,single_list); //이전 노드 결과 값 저장 
